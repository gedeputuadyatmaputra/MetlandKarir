<?php 
    ...
    if(isset($_GET['id'])){
        ...
        $sql_get_file = "SELECT upload_cv FROM tb_intern WHERE id_intern = ?";
        ...
        if ($file_cv) {
            $file_path = '../../assets/cv_intern/' . $file_cv;
            if (file_exists($file_path)) {
                unlink($file_path); // Menghapus file
            }
        }

        $sql_hapus = "DELETE FROM tb_intern WHERE id_intern = ?";
...
header("Location: data_intern.php");
exit();