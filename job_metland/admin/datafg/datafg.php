...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <?php endif; ?>
     ...
     <form method="GET" class="mb-3">
        <div class="row">
            ...
            <div class="col-md-3 mt-4">
                <input type="text" name="joblamar" class="form-control" placeholder="Lamar Jabatan" value="<?php echo isset($_GET['joblamar']) ? $_GET['joblamar'] : ''; ?>">
            </div>
    ...
    <?php 
        ...
        if ($pendidikan) {
            $sql .= " AND pendidikan_fg = '" . mysqli_real_escape_string($conn, $pendidikan) . "'";
        }
        if ($universitas) {
        ...
        if ($jabatan) {
            $sql .= " AND (jabatan_organisasi LIKE '%" . mysqli_real_escape_string($conn, $jabatan) . "%' OR posisi_kerja LIKE '%" .  mysqli_real_escape_string($conn, $jabatan) . "%')";
        }
        if ($masakerja) {
    ...
    <h4 class="fw-bold py-3 mb-4">Data Calon Karyawan Fresh Graduate</h4>
            ...
            <table class="table table-bordered text-center">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="../../assets/cv_freshgrad/<?php echo $row['upload_cv']; ?>" class="btn btn-success btn-sm" target="_blank">Lihat CV</a> | 
                            <a href="../../assets/cv_freshgrad/<?php echo $row['upload_cv']; ?>" class="btn btn-warning btn-sm" download>Download CV</a>
                        </td>
                        <td>
                            <a href="lhtfg.php?id=<?php echo $row['id_fg']; ?>" class="btn btn-warning btn-sm" onclick="return confirm('Apakah Ingin Lihat Data ?')">Lihat</a>
                            <br>
                            <a href="delfg.php?id=<?php echo $row['id_fg']; ?>" class="btn btn-danger btn-sm mt-4" onclick="return confirm('Apakah Ingin Hapus Data ?')">Hapus</a>
                        </td>
                    </tr>
                    <?php endwhile; ?>
                </tbody>
